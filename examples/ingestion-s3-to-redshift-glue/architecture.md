# Architecture - Ingestion S3 to Redshift Glue

## Purpose

This note explains how the `ingestion-s3-to-redshift-glue` example aligns to the repository architecture system.
...
That role is intentionally narrower than DP-EH processing. Redshift receives Gold-ready data after Glue processing has already completed.

## Incremental Load Design Notes

This example uses a controlled incremental load pattern:

- Glue produces the current Gold-ready batch outside Redshift
...
- it preserves Glue as the Transformation engine
