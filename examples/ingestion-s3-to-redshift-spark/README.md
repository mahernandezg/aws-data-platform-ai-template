# Ingestion S3 to Redshift Spark Example
...
This example is intended to be the Redshift-serving companion to the Iceberg-and-Athena Spark example.
...
This example keeps all Transformation logic in Spark and Apache Iceberg.
...
- replace Spark as the enterprise processing layer
...
- Spark writes the current serving increment for affected business keys to a controlled S3 staging path
...
```text
examples/ingestion-s3-to-redshift-spark/
|-- README.md
|-- architecture.md
|-- environments.md
|-- redshift-loading.md
...
- IAM Roles only
- no IAM Users
- role-based Spark runtime access
