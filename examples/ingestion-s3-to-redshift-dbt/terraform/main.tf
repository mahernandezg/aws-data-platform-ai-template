...
# Purpose: Near-runnable Terraform starter for the ingestion-s3-to-redshift-dbt example covering ISC Landing Zone, DP-EH dbt-oriented Iceberg processing, controlled S3 staging for Redshift loads, and DDC Redshift serving.
...
locals {
  ...
  standard_tags = {
    ...
    example_pattern = "ingestion-s3-to-redshift-dbt"
  }
}
...
data "aws_iam_policy_document" "dp_eh_dbt_runtime" {
  statement {
...
module "dp_eh_dbt_runtime_role" {
  source = "../../../templates/terraform/modules/dcs_iam_role"

  project                = var.project
  component              = "dcs"
  environment            = var.environment
  role_purpose           = "dp-eh-dbt-runtime"
  assume_role_principals = ["ec2.amazonaws.com"]
  inline_policy_json     = data.aws_iam_policy_document.dp_eh_dbt_runtime.json

  tags = {
    workload_component = "DP-EH"
    workload_type      = "dbt-execution"
